...
<script>
  ...
  $.ajax({
    ...
    data: '{"UserID": "5", "marketplace": "'+marketplace+'", "merchant_name": "Twinzahra Shop"  , "order_id": "'+order_number+'", "name": "'+name+'", "shipping_provider": "'+shipping_provider+'", "tracking_code": "'+tracking_code+'", "shipping_amount": "'+shipping_amount+'", "tracking_code_pre": "'+tracking_code_pre+'", "remark": "'+remark+'", "payment_method": "'+payment_method+'" , "TokenSession": "<?= session() ->get('HTTP_TOKEN') ?>"}',
             url:'<?= url('api/orders.php?request=created_orders') ?>',
            beforeSend: function () {
...
<script>
    function sound_error() {
        ...
        audioElement.setAttribute('src', '<?= env('APP_URL') . ('resources/music/beep_error.mp3') ?>');
     ...
     function sound_add() {
        var audioElement = document.createElement('audio');
        audioElement.setAttribute('src', '<?= env('APP_URL') . ('resources/music/beep_add.mp3') ?>');
        audioElement.setAttribute('autoplay', 'autoplay');
        audioElement.load();
        audioElement.play();
    }
 ...
 $.ajax({
    ...
    processData: false,
    data: '{"UserID": "5"}',
    url:'<?= url('api/orders.php?request=get_cart_details') ?>',
 ...
 function loadDataItem(){
  ...
  $('#ResultItemOrders').html(createSkeleton(displayProduct));
   ...
   headers: {
      ...
      $.ajax({
        url:'<?= url('cashier/load_item_products') ?>',
        method:"POST",
...
if(e.which == 13){
 ...
 $.ajax({
    ...
    url:'<?= url('api/orders.php?request=add_cart') ?>',
  ...
  var text;
   ...
   headers: {
    ...
    data: '{"CartDetailID": "'+ CartDetailID +'"}',
      url:'<?= url('api/orders.php?request=delete_cart_details') ?>',
